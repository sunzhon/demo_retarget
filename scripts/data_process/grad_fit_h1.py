...
if __name__ == "__main__":
    ...
    # Initialize SMPL parser with neutral
    smpl_parser_n = SMPL_Parser(model_path="data/smpl", gender="neutral")
    smpl_parser_n.to(device)

    # Load optimized shape
    ...
    for data_key in pbar:
        ...
        for iteration in range(500):
            ...
            # the verts shape is (N, 6890, 3)
            # the joints shape is (N, 24, 3)
            verts, joints = smpl_parser_n.get_joints_verts(pose_aa_walk, shape_new, trans)
            
            # its shape is (1, N, 22, 3)
            pose_aa_h1_new = torch.cat([gt_root_rot[None, :, None], \
            ...
            # the shape of diff is (1, N, 11, 3), joints[:, smpl_joint_pick_idx] is (N, 11, 3)
