...
class MotionLibK1(MotionLibBase):

    def __init__(self, motion_file, device, fix_height=FixHeightMode.no_fix,
                  masterfoot_conifg=None, min_length=-1, im_eval=False,
                    multi_thread=True, extend_hand = True, extend_head = False, 
                 mjcf_file="resources/robots/h1/h1.xml", sim_timestep = 1/50):
    ...
    def load_motions(self, skeleton_trees, 
                     gender_betas, limb_weights, 
                     random_sample=True, start_idx=0,
                       max_len=-1, target_heading = None):
        """
        Description:
            Load the motion data from the file and move it to the GPU
        Args:
            skeleton_trees: list of skeleton trees, each element is a skeleton tree
            gender_betas:
            limb_weights:
        ...
        if flags.real_traj:
            # q_gts = quest global translation
            # q_grs = quest global rotation
            # q_gavs = quest global angular velocity
            # q_gvs = quest global velocity
        ...
        for f in tqdm(range(len(res_acc))):
            ...
            del curr_motion
        # ZL: load the motion data to the GPU
    ...
    @staticmethod
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, gender_betas,
                                   fix_height, mesh_parsers, masterfoot_config,  target_heading, max_len, queue, pid):
        # ZL: loading motion with the specified skeleton. Perfoming forward kinematics to get the joint positions
        np.random.seed(np.random.randint(5000)* pid)
        res = {}
        assert (len(ids) == len(motion_data_list))
