...
class Humanoid_Batch:
    ...
    def __init__(self, mjcf_file = f"/home/admin-1/workspace/kepler_ws/resources/Robots/Kepler/K1/mjcf/mjmodel.xml",
                  extend_hand = True, extend_head = True, device = torch.device("cpu")):
        self.mjcf_data = mjcf_data = self.from_mjcf(mjcf_file)
        self.extend_hand = extend_hand
        self.extend_head = extend_head
        # 
        if extend_hand: # extend_hand is ["left_hand_link", "right_hand_link"]
            self.model_names = mjcf_data['node_names'] + ["left_hand_link", "right_hand_link"]
            self._parents = torch.cat((mjcf_data['parent_indices'], torch.tensor([19, 24]))).to(device) # Adding the hands joints
            arm_length = 0.241
            ...
            self._remove_idx = 2 # because we added two hands
        else:
        ...
        if extend_head:
            self._remove_idx = 3 # because we added head and hands
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        if self.extend_hand and self.extend_head and pose.shape[-2] == (len(self._parents)-1): # joint number is node number - 1
